...
export class Game {
  ...
  private writeHelpMessage(): void {
    let helpMessage = [
      ...
      `Watch out for %c{${this.pedroColor}}Pineapple Thief%c{}!`,
    ]
  ...
  private getActorName(actor: Actor): string {
    switch (actor.type) {
      ...
      case ActorType.Pedro:
        return `%c{${actor.glyph.foregroundColor}}Pineapple Thief%c{}`
      case ActorType.TinyPedro:
        return `%c{${actor.glyph.foregroundColor}}Pineapple Thief's son%c{}`
      default:
